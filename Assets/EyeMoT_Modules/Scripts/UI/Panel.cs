...
namespace EyeMoT
{
    public class Panel : MonoBehaviour
    {
            ...
            float to_alpha, Tween.EasingFunction func, Action whenDone)
        {
            ...
            for (float f = immediately ? 1 : 0; immediately || Forward(ref f, Time.deltaTime / _easeTime);)
            {
        ...
        public bool Forward(ref float f, float step)
        {
            if (f < 0)
            {
                f = 0;
        ...
        public bool Backwards(ref float f, float step)
        {
            if (f > 1)
            {
                f = 1;
